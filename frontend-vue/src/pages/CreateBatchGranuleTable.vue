...
<template>
  <div>
    <div class="mb-2 flex items-center justify-between">
      <span class="text-xs text-muted">数据粒 · {{ rows.length }} 条</span>
      <div class="flex gap-1.5 text-xs">
        <button
          type="button"
          @click="emit('openCsv')"
          class="rounded-md border border-border bg-surface px-2 py-1 text-muted transition hover:border-accent/40 hover:text-text"
        >
          粘贴 CSV
        </button>
        <button
      ...
      <table class="w-full text-xs">
        <thead class="bg-subtle/60 th-row">
          ...
          <tr class="text-[10px] normal-case">
            ...
            <template v-for="s in schema.slots" :key="`sub-${s.name}`">
              <th class="px-2 py-1 text-muted">url</th>
              <th class="px-2 py-1 text-muted">filename</th>
              <th v-if="!s.credential" class="px-2 py-1 text-muted">credential</th>
            </template>
            <th
              v-for="m in schema.metaFields"
              :key="`sub-meta-${m.name}`"
        ...
        <tbody>
          ...
          <tr v-if="rows.length === 0">
            <td :colspan="99" class="px-4 py-4 text-center text-muted">
