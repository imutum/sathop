  ...
  <div>
    <div class="mb-1.5 flex items-baseline gap-2">
      ...
      <RouterLink
        ...
        class="text-[10.5px] text-muted transition hover:text-accent"
      >
        全屏 →
      </RouterLink>
    </div>
    <div v-if="q.isLoading.value" class="text-[11px] text-muted">加载中…</div>
    <div v-else-if="rows.length === 0" class="text-[11px] text-muted">暂无事件</div>
    <ul
      v-else
      class="max-h-56 space-y-1 overflow-auto rounded-lg border border-border/60 bg-bg/40 p-2 font-mono"
    >
      <li v-for="e in rows" :key="e.id" class="flex items-start gap-2 text-[11px]">
        <span class="w-16 shrink-0 text-muted">{{ fmtAge(e.ts) }}</span>
        <Badge :tone="e.level" dot>{{ levelLabel(e.level) }}</Badge>
        <span class="w-24 shrink-0 truncate text-muted" :title="e.source">{{ e.source }}</span>
