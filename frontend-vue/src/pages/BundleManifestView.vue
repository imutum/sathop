...
import BundleFileBrowser from "./BundleFileBrowser.vue";
  ...
  <div class="space-y-5 text-sm">
    <div class="flex items-start justify-between gap-3">
      <div>
        ...
        <div v-if="d.description" class="mt-1.5 text-xs text-muted">{{ d.description }}</div>
      </div>
      <div class="flex items-center gap-2">
    ...
    <BundleSection title="输入槽位 · slots" :count="slots.length">
      <div v-if="slots.length === 0" class="text-xs text-muted">未声明</div>
    ...
    >
      <div class="flex flex-wrap gap-1.5">
        <RouterLink
          ...
          :key="n"
          to="/shared"
          class="inline-flex items-center gap-1 rounded-md bg-subtle px-2 py-1 font-mono text-[11px] text-muted transition hover:bg-accent-soft hover:text-accent"
          title="跳转到「共享文件」页"
        >
          {{ n }}
    ...
    <BundleSection title="Python 依赖 · pip" :count="pip.length">
      <div v-if="pip.length === 0" class="text-xs text-muted">无</div>
      ...
      <table class="w-full font-mono text-[11.5px]">
        <tbody>
          <tr v-for="[k, v] in envEntries" :key="k" class="border-t border-border/50">
            <td class="py-1.5 pr-3 text-muted">{{ k }}</td>
