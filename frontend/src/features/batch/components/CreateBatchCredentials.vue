...
<template>
  <fieldset class="space-y-2">
    ...
    <div class="space-y-3 rounded-lg border border-border bg-legacy-subtle/40 p-3">
      <div
        v-for="name in names"
        :key="name"
      ...
      >
        ...
        <TextInput
          ...
          placeholder="用户名"
        />
        <div v-else class="text-legacy-muted">—</div>
        <TextInput
          :id="`cred-${name}-secret`"
        ...
        <div class="flex items-center gap-2 whitespace-nowrap">
          <label
            ...
            class="flex items-center gap-1 text-[11px] text-legacy-muted"
          ...
          <button
            ...
            class="text-[11px] text-legacy-muted hover:text-danger"
            title="从本浏览器删除已保存的凭证"
          >
            清除
          </button>
